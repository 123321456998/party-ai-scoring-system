...
const partialScorePattern = /^\d{0,2}$/
...
export function ScorePage() {
  ...
  if (loading) return <AppShell pageLabel="匿名评委评分" showNavigation={false} showBrandSubtitle={false}><div className="page-container loading-state">正在恢复匿名评分身份…</div></AppShell>
  if (identityError || showRecovery || !scorecard) return <><AppShell pageLabel="匿名评委评分" showNavigation={false} showBrandSubtitle={false}><IdentityGate onClaim={async () => { setShowRecovery(false); await loadScorecard() }} /></AppShell>{scorecard && <button className="recovery-dismiss" onClick={() => setShowRecovery(false)}>返回当前评分</button>}</>

  if (scorecard.eventStatus === 'prepare') return <AppShell pageLabel="匿名评委评分" showNavigation={false} showBrandSubtitle={false}><div className="score-page page-container"><div className="score-intro"><div className="eyebrow"><ShieldCheck size={15} />匿名评委评分</div><h1>评分尚未开始</h1><div className="status-message"><LockKeyhole size={17} /><span>评分尚未开始，请等待工作人员开启评分。</span></div></div></div></AppShell>

  const completed = scorecard.completedCount
  const isLocked = scorecard.eventStatus === 'locked' || scorecard.eventStatus === 'published'
  return <AppShell pageLabel="匿名评委评分" showNavigation={false} showBrandSubtitle={false}><div className="score-page page-container"><div className="score-intro"><div className="eyebrow"><ShieldCheck size={15} />匿名评委评分 {isDemoMode && <span className="mode-chip">DEMO</span>}</div><h1>{isLocked ? '评分已结束' : '请为参赛队伍评分'}</h1>{isLocked && <div className="locked-note"><LockKeyhole size={16} /><div><strong>评分已结束，您的最终评分已锁定。</strong><span>当前页面仅供查看。</span></div></div>}<div className="progress-line"><span>已完成 <strong>{completed}</strong> / {eventConfig.teams.length}</span><div className="progress-track"><i style={{ width: `${completed / eventConfig.teams.length * 100}%` }} /></div></div></div><div className="score-list">{scorecard.teams.map((team, index) => <article className={`score-card ${team.score !== null ? 'is-complete' : ''}`} key={team.id}><div className="team-info"><div className="team-heading"><div className="team-index">{String(index + 1).padStart(2, '0')}</div><div className="team-name">{team.name}</div></div><ScoreStatus submitted={Boolean(scorecard.submittedAt)} state={isLocked ? 'locked' : states[team.id] || (team.score !== null ? 'saved' : 'idle')} /></div><div className="score-input-wrap"><input aria-label={`${team.name}评分`} placeholder="—" value={values[team.id] ?? ''} readOnly={isLocked || Boolean(scorecard.submittedAt)} onChange={(event) => { const next = event.target.value; scheduleSave(team.id, next); if (/^\d{0,2}$/.test(next)) setValues((current) => ({ ...current, [team.id]: next })) }} onBlur={() => normalizeOnBlur(team.id)} inputMode="numeric" type="text" maxLength={2} /><span>分 / {eventConfig.fullScore}</span></div></article>)}</div>{submissionNotice && !isLocked && <div className="submission-notice"><Check size={17} /><strong>评分已提交，请等待工作人员通知</strong></div>}{!isLocked && !scorecard.submittedAt && <div className="score-submit-block"><button className="primary-button score-submit-button" disabled={completed !== eventConfig.teams.length || submitting} onClick={() => void confirmSubmit()}>{submitting ? '提交中…' : '确认提交评分'}</button>{completed !== eventConfig.teams.length && <p>请完成全部队伍评分后提交</p>}{submitError && <div className="save-error"><CircleAlert size={14} />{submitError}</div>}</div>}{!isLocked && completed === eventConfig.teams.length && !scorecard.submittedAt && <div className="complete-note"><Check size={16} /><div><strong>您已完成全部参赛队伍评分</strong><span>提交后将不能继续修改评分。</span></div></div>}<div className="score-actions"><div className="score-note"><LockKeyhole size={15} />{isLocked ? '评分已锁定，当前页面仅供查看。' : scorecard.submittedAt ? '评分已确认提交，当前页面仅供查看。' : '评分仅与当前匿名身份关联，不显示其他评委数据。'}</div><button className="recovery-link" onClick={() => void exitJudgeIdentity()}><LogOut size={14} />退出评委身份</button></div>{isDemoMode && <div className="demo-inline">开发演示模式 · 数据仅保存在当前浏览器</div>}</div></AppShell>
}

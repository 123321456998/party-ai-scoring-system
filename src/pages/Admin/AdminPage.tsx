...
export function AdminPage() {
  ...
  return <AppShell pageLabel="现场评分控制台"><div className="admin-page page-container">
    <div className="admin-heading"><div className="admin-brand-area"><div className="admin-logo-slot" aria-label="单位高清Logo预留位置" /><div><div className="eyebrow">党建引领·数智赋能 {isDemoMode && <span className="mode-chip">DEMO</span>}</div><h1>AI创新竞赛</h1><p>现场评分控制台</p></div></div><div className={`status-pill status-${dashboard.status}`}><span />{isPrepare ? '赛前准备' : isScoring ? '评分进行中' : isLocked ? '评分已锁定' : '最终成绩已公布'}</div></div>

...
export function AdminPage() {
  ...
  return <AppShell pageLabel="现场评分控制台"><div className="admin-page page-container">
    ...
    <div className="admin-grid admin-grid-top">
      ...
      <section className="panel shared-access-panel"><SectionHeading eyebrow="SHARED ACCESS" title="统一评分入口二维码" description="所有评委扫码进入首页，再输入工作人员提供的评分码" /><div className="shared-access-content"><div className="qr-grid"><QrCodeCard /></div><div className="shared-access-actions"><button className="secondary-button" onClick={() => window.print()}>打印二维码</button><div className="code-warning"><CircleAlert size={14} />二维码不绑定评委身份。请将评分码分别提供给评委，扫码后在首页输入评分码。</div><button className="secondary-button" onClick={() => void navigator.clipboard?.writeText(dashboard.codes.join('\n'))}><Clipboard size={16} />复制评分码</button>{isPrepare && dashboard.codes.length === 0 && <button className="secondary-button" onClick={() => void act(generateAnonymousCodes, '评分码已生成，请分别提供给评委。')}><RefreshCw size={16} />生成评分码</button>}</div></div></section>
    </div>

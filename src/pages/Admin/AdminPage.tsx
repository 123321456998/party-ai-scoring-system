...
import { adminLogin, demoAdminPin, generateAnonymousCodes, getAdminDashboard, isDemoMode, lockAndFinalize, publishResults, resetDemoCompetition, resetEvent, saveEventConfig, startScoring, subscribeToScoringUpdates, type AdminDashboard, type TeamConfig } from '../../data/scoringRepository'
...
export function AdminPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const next = await getAdminDashboard(); setDashboard(next); setName(next.name); setTeams(next.teams); setJudges(next.expectedJudges); setLoginError(false) } catch (cause) { window.alert(cause instanceof Error ? cause.message : '读取控制台失败。'); setLoginError(true) } finally { setLoading(false) } }, [])
  ...
  return <AppShell pageLabel="现场评分控制台"><div className="admin-page page-container">
    ...
    <div className="admin-grid admin-grid-top">
      <section className="panel"><SectionHeading eyebrow="EVENT SETUP" title="赛事设置" description={isPrepare ? 'prepare 状态下可以修改赛事基础信息' : '评分开始后赛事设置已锁定'} /><div className="settings-form"><label>赛事名称<input value={name} onChange={(event) => setName(event.target.value)} disabled={!isPrepare} /></label><label>匿名评委人数<input value={judges} onChange={(event) => setJudges(Number(event.target.value))} disabled={!isPrepare} type="number" min="1" max="99" /></label><div className="team-settings"><span>参赛队伍</span>{teams.map((team, index) => <label key={team.id}><b>{String.fromCharCode(65 + index)}</b><input value={team.name} onChange={(event) => setTeams((current) => current.map((item) => item.id === team.id ? { ...item, name: event.target.value } : item))} disabled={!isPrepare} /></label>)}</div>{isPrepare && <button className="secondary-button" onClick={() => void act(() => saveEventConfig({ name, teams: teams.map(({ code, name: teamName }) => ({ code, name: teamName })), expectedJudges: judges }), '赛事设置已保存。')}><Save size={16} />保存赛事设置</button>}</div></section>
    ...
    {isScoring && !dashboard.canLock && <div className="lock-reasons"><CircleAlert size={15} /><div><strong>至少需要一位评委确认提交全部评分后才能生成最终结果。</strong>{dashboard.lockReasons.slice(0, 4).map((reason) => <span key={reason}>{reason}</span>)}</div></div>}

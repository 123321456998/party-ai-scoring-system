...
function QrCodeCard({ code, index, baseUrl }: { code: string; index: number; baseUrl: string }) {
  ...
  const download = () => {
    ...
    link.download = `匿名评委${String(index + 1).padStart(2, '0')}-评分二维码.png`
    link.href = canvas.toDataURL('image/png')
    link.click()
  }
  return <div className="qr-card" data-qr-code={code}><QRCodeCanvas value={scoreUrl} size={156} includeMargin /><strong>匿名评委 {String(index + 1).padStart(2, '0')}</strong><code>{code}</code><button className="text-button" onClick={download}>下载此二维码</button></div>
}
...
export function AdminPage() {
  ...
  return <AppShell pageLabel="现场评分控制台"><div className="admin-page page-container">
    ...
    <div className="admin-grid admin-grid-top">
      ...
      <section className="panel"><SectionHeading eyebrow="ANONYMOUS ACCESS" title="评委二维码" description="每位评委一张，扫码后自动进入对应匿名评分身份" />{dashboard.codes.length ? <><label className="qr-url-field">二维码访问地址<input value={qrBaseUrl} onChange={(event) => setQrBaseUrl(event.target.value)} placeholder="https://正式网址.example" /></label><div className="qr-url-note">打印前请确认这里是评委手机能够访问的正式网址，不要使用 localhost。</div><div className="qr-grid">{dashboard.codes.map((code, index) => <QrCodeCard key={code} code={code} index={index} baseUrl={qrBaseUrl} />)}</div><button className="secondary-button" onClick={() => window.print()}>打印全部二维码</button><div className="code-warning"><CircleAlert size={14} />二维码对应匿名评委序号，请分别打印并放在对应评委桌上；评分码仍可作为备用。</div><button className="secondary-button" onClick={() => void navigator.clipboard?.writeText(dashboard.codes.join('\n'))}><Clipboard size={16} />复制备用评分码</button></> : <div className="empty-code"><KeyRound size={24} /><span>尚未生成评委二维码</span></div>}{isPrepare && dashboard.codes.length === 0 && <button className="secondary-button" onClick={() => void act(generateAnonymousCodes, '匿名评分码已生成，请检查二维码访问地址后下载或打印。')}><RefreshCw size={16} />生成评委二维码</button>}</section>
    </div>
    <div className="admin-actions"><div className="action-copy">{isPrepare ? '完成赛事设置和评委二维码后即可开启评分。' : isScoring ? '所有评分完成后才能锁定并生成最终结果。' : isLocked ? '评分已锁定，最终成绩尚未公布。' : '最终成绩已公布，评委端只读。'}</div>{isPrepare && <button className="primary-button action-button" disabled={!dashboard.name || dashboard.codes.length !== dashboard.expectedJudges} onClick={() => ask('start')}><Play size={17} />开始评分</button>}{isScoring && <button className="primary-button action-button" disabled={!dashboard.canLock} onClick={() => ask('lock')}><Lock size={17} />锁定评分并生成结果</button>}{isLocked && <><span className="locked-action-label">评分已锁定</span><button className="primary-button action-button" disabled={!dashboard.canPublish} onClick={() => ask('publish')}>公布最终成绩</button></>}{isPublished && <a className="secondary-button open-results-button" href="/results">打开成绩大屏</a>}{isDemoMode && <button className="text-button" onClick={() => ask('reset')}>DEMO 重置比赛</button>}</div>

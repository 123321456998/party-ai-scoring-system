...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: teams }, { data: judges }, { data: sessions }, { data: scores }, { data: finalResults }] = await Promise.all([
      db.from('teams').select('id,team_code,name').eq('event_id', event.id),
    ...
    const submittedJudges = judgeRows.filter((judge) => Boolean(judge.submitted_at)).length
    ...
    const teamProgress = teamRows.map((team) => ({ teamId: team.id, teamName: displayTeamName(team), count: scoreRows.filter((score) => actualParticipantIds.has(score.anonymous_judge_id) && score.team_id === team.id).length, expected: actualParticipants }))
    const expectedScores = actualParticipants * teamRows.length
    const incompleteSubmitted = judgeProgress.filter((judge) => judge.status === 'submitted' && !judge.completed)
    const lockReasons = submittedJudges === 0 ? ['至少需要一位评委确认提交全部评分。'] : incompleteSubmitted.map((judge) => `${judge.label}的评分尚未完整。`)
    const result = { name: event.name, subtitle: '党建引领 · 数智赋能', status: event.status, expectedJudges: event.expected_judges, teams: teamRows.map((team) => ({ id: team.id, code: team.team_code, name: displayTeamName(team) })), codes: [], preGeneratedCodes: judgeRows.length, actualParticipants, submittedJudges, inProgressJudges, unusedJudges, judgeProgress, teamProgress, totalScores: scoreRows.length, expectedScores, completedJudges: submittedJudges, canLock: event.status === 'scoring' && submittedJudges > 0 && lockReasons.length === 0, canPublish: event.status === 'locked' && finalRows.length === teamRows.length, lockReasons, finalResults: finalRows.map((row) => { const team = teamRows.find((item) => item.id === row.team_id); return { teamId: row.team_id, teamName: team ? displayTeamName(team) : '', averageScore: Number(row.average_score), scoreCount: row.score_count, rankPosition: row.rank_position, award: row.award, tie: row.tie } }), hasTies: finalRows.some((row) => row.tie) }

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: teams, error: teamsError }, { data: scores, error: scoresError }] = await Promise.all([db.from('teams').select('id,team_code,name').eq('event_id', event.id).eq('active', true), db.from('scores').select('team_id,score').eq('event_id', event.id).eq('anonymous_judge_id', session.anonymous_judge_id)])
    if (teamsError || scoresError) throw teamsError ?? scoresError
    const scoreMap = new Map((scores ?? []).map((score) => [score.team_id, score.score])); const result = sortTeamsForDisplay(teams ?? []).map((team) => ({ id: team.id, code: team.team_code, name: displayTeamName(team), score: scoreMap.get(team.id) ?? null }))

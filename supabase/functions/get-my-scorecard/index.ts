...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ eventName: event.name, eventStatus: event.status, teams: result, completedCount: result.filter((team) => team.score !== null).length }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })

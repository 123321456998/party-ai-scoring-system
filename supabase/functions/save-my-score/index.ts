...
Deno.serve(async (req) => {
  ...
  try {
    const { team_id, score } = await req.json(); if (typeof team_id !== 'string' || typeof score !== 'number' || !Number.isInteger(score) || score < 0 || score > 10) throw new Error('评分必须是 0 到 10 之间的整数。')
